...
export const chipOff = "border-[var(--border)] text-[var(--muted)] hover:border-[var(--nimble)] hover:text-[var(--text)]";
export const chipOn = "border-[var(--nimble)] bg-[var(--panel-2)] text-[#9fe3bd]";
export const nameCls = "text-base font-bold uppercase tracking-[0.12em] text-[#9fe3bd]";
...
export const hbBadge = "rounded border border-[var(--nimble)] px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-[0.12em] text-[#9fe3bd]";
...
export function SearchForm({ base, q, placeholder, hidden }: { base: string; q: string; placeholder: string; hidden: Query }) {
  return (
    <form method="get" action={base} className="mb-4 flex gap-2">
      <input type="search" name="q" defaultValue={q} placeholder={placeholder} className="min-w-0 flex-1 rounded border border-[var(--border)] bg-[var(--panel)] px-3 py-2.5 text-sm text-[var(--text)] outline-none placeholder:text-[var(--muted)] focus:border-[var(--nimble)]" />
      {Object.entries(hidden).map(([k, v]) => (v ? <input key={k} type="hidden" name={k} value={v} /> : null))}
      <button className="shrink-0 rounded border border-[var(--border)] px-4 py-2.5 text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--muted)] hover:border-[var(--nimble)] hover:text-[var(--text)]">Search</button>
    </form>
  );
}
...
export function CountLine({ count, noun, base, filtered }: { count: number; noun: string; base: string; filtered: boolean }) {
  return (
    <div className="mb-4 mt-3 flex items-center gap-3 text-[11px] uppercase tracking-[0.15em] text-[var(--muted)]">
      ...
      {filtered ? <Link href={base} className="text-[var(--nimble)] hover:underline">Clear filters</Link> : null}
    </div>
  );
}
export function EmptyState({ noun, base }: { noun: string; base: string }) {
  return (
    <div className="rounded-lg border border-[var(--border)] bg-[var(--panel)] p-6"><h2 className="text-base font-bold uppercase tracking-[0.15em]">Nothing found</h2>
      <p className="mt-3 text-sm leading-relaxed text-[var(--muted)]">No {noun} matches those filters. Try a broader search or <Link href={base} className="text-[var(--nimble)] underline">clear them</Link>.</p></div>
  );
}
