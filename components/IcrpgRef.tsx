...
// Shared shell for the ICRPG (Index Card RPG) reference pages (app/icrpg/*).
// Same pieces as D62eRef / SwRef — header with "My Homebrew" + "← Home" links,
// search form, filter chips, count line, empty state, cards — in the ICRPG
// burnt-orange accent.
...
export const WORLDS: { key: string; label: string }[] = [
  { key: "core", label: "Core" },
  { key: "alfheim", label: "Alfheim" },
  { key: "warpshell", label: "Warp Shell" },
  { key: "ghostmountain", label: "Ghost Mountain" },
  { key: "vigilantecity", label: "Vigilante City" },
  { key: "bloodandsnow", label: "Blood & Snow" },
];
const WORLD_NAME: Record<string, string> = Object.fromEntries(WORLDS.map((w) => [w.key, w.label]));
export function worldName(key: string): string {
  return WORLD_NAME[key] ?? key;
}
...
export function IcrpgHeader({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <header className="mb-8 flex items-end justify-between gap-4 border-b border-[var(--border)] pb-6">
      <div>
        ...
        <p className="mt-1 text-[13px] font-semibold uppercase tracking-[0.25em] text-[var(--icrpg)] sm:text-[11px] sm:tracking-[0.35em]">{subtitle}</p>
      </div>
      <div className="flex shrink-0 gap-2">
        <Link href="/icrpg/homebrew" className="rounded border border-[var(--border)] px-4 py-2.5 text-[13px] font-semibold uppercase tracking-[0.15em] text-[var(--muted)] hover:border-[var(--icrpg)] hover:text-[var(--text)] sm:px-3 sm:py-1.5 sm:text-[11px]">My Homebrew</Link>
...
export function SearchForm({ base, q, placeholder, hidden }: { base: string; q: string; placeholder: string; hidden: Query }) {
  return (
    <form method="get" action={base} className="mb-4 flex gap-2">
      <input type="search" name="q" defaultValue={q} placeholder={placeholder} className="min-w-0 flex-1 rounded border border-[var(--border)] bg-[var(--panel)] px-3 py-2.5 text-sm text-[var(--text)] outline-none placeholder:text-[var(--muted)] focus:border-[var(--icrpg)]" />
      {Object.entries(hidden).map(([k, v]) => (v ? <input key={k} type="hidden" name={k} value={v} /> : null))}
      <button className="shrink-0 rounded border border-[var(--border)] px-4 py-2.5 text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--muted)] hover:border-[var(--icrpg)] hover:text-[var(--text)]">Search</button>
    </form>
  );
}
...
export function CountLine({ count, noun, base, filtered }: { count: number; noun: string; base: string; filtered: boolean }) {
  return (
    <div className="mb-4 mt-3 flex items-center gap-3 text-[11px] uppercase tracking-[0.15em] text-[var(--muted)]">
      ...
      {filtered ? <Link href={base} className="text-[var(--icrpg)] hover:underline">Clear filters</Link> : null}
    </div>
  );
}
...
export function EmptyState({ noun, base }: { noun: string; base: string }) {
  return (
    <div className="rounded-lg border border-[var(--border)] bg-[var(--panel)] p-6">
      ...
      <p className="mt-3 text-sm leading-relaxed text-[var(--muted)]">No {noun} matches those filters. Try a broader search or <Link href={base} className="text-[var(--icrpg)] underline">clear them</Link>.</p>
    </div>
  );
}
