...
// Shared shell for the DarkSpace reference pages (app/darkspace/*) — the same
// pieces as NimbleRef / SwRef, in the DarkSpace sci-fi cyan.
...
// A small typed table used across the equipment / starship pages.
export function DataTable({ head, rows }: { head: string[]; rows: (string | number)[][] }) {
  return (
    <div className="overflow-x-auto">
      <table className="w-full border-collapse text-[12px]">
        <thead>
          <tr>{head.map((h) => <th key={h} className="border-b border-[var(--darkspace)] px-2 py-1.5 text-left text-[9px] font-black uppercase tracking-[0.1em] text-[#8fd6ea]">{h}</th>)}</tr>
        </thead>
        <tbody>
          {rows.map((r, i) => (
            <tr key={i}>{r.map((c, j) => <td key={j} className="border-b border-[var(--border)] px-2 py-1.5 align-top text-[var(--text)]">{c}</td>)}</tr>
          ))}
        </tbody>
      </table>
    </div>
