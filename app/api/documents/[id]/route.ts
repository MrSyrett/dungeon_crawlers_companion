...
function extractDocTitle(tool: string, data: object): string | null {
  try {
    ...
    if (tool === "icrpg-character" && typeof blob.icrpg_sheet === "string") {
    ...
    if (tool === "mmrpg-character" && typeof blob.mmrpg_sheet === "string") {
      const sheet = JSON.parse(blob.mmrpg_sheet) as { name?: unknown };
      if (typeof sheet.name === "string" && sheet.name.trim()) return sheet.name.trim();
    }
    if (tool === "jlu-character" && typeof blob.jlu_sheet === "string") {
      const sheet = JSON.parse(blob.jlu_sheet) as { name?: unknown };
      if (typeof sheet.name === "string" && sheet.name.trim()) return sheet.name.trim();
    }
...
function extractLinkedCampaignId(data: object): string | null {
  try {
    ...
    for (const key of ["dcc_sheet", "ace_sheet", "kob_sheet", "nimble_sheet", "sw_sheet", "dnd_sheet", "d62e_sheet", "icrpg_sheet", "yze_sheet", "mmrpg_sheet", "jlu_sheet"]) {
