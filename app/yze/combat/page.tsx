  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-5 py-10">
      ...
      <section className="mb-8">
        <h2 className="mb-3 text-base font-bold uppercase tracking-[0.12em] text-[#3fd0e6]">Conditions</h2>
            ...
            <ul className="flex flex-col gap-2">
              {phys.map((c) => (
                <li key={c.name} className="rounded-lg border border-[var(--border)] bg-[var(--panel)] p-3">
                  <span className="text-[14px] font-bold text-[#3fd0e6]">{c.name}</span>
      ...
      <section className="mb-8">
        <h2 className="mb-3 text-base font-bold uppercase tracking-[0.12em] text-[#3fd0e6]">Critical injuries — physical (D66)</h2>
            ...
            <tbody>
              {YZE_CRITS_PHYSICAL.map((c) => (
                <tr key={c.roll} className="border-t border-[var(--border)]">
                  ...
                  <td className="p-2 font-semibold text-[#3fd0e6]">{c.injury}</td>
      ...
      <section className="mb-8">
        <h2 className="mb-3 text-base font-bold uppercase tracking-[0.12em] text-[#3fd0e6]">Critical injuries — mental (D66)</h2>
        <div className="overflow-x-auto rounded-lg border border-[var(--border)]">
          <table className="w-full border-collapse text-[12px]">
            <thead>
              <tr className="bg-[var(--panel-2)] text-left uppercase tracking-[0.1em] text-[var(--muted)]">
            ...
            <tbody>
              {YZE_CRITS_MENTAL.map((c) => (
                <tr key={c.roll} className="border-t border-[var(--border)]">
                  <td className="p-2 font-mono text-[var(--muted)]">{c.roll}</td>
                  <td className="p-2 font-semibold text-[#3fd0e6]">{c.trauma}</td>
                  <td className="p-2 text-[var(--text)]">{c.effect}</td>
                  <td className="p-2 font-mono text-[var(--muted)]">{c.healing}</td>
                </tr>
              ))}
      ...
      <section>
        <h2 className="mb-3 text-base font-bold uppercase tracking-[0.12em] text-[#3fd0e6]">Panic table</h2>
            ...
            <tbody>
              {YZE_PANIC.map((p) => (
                <tr key={p.roll} className="border-t border-[var(--border)]">
                  ...
                  <td className="p-2 font-semibold text-[#3fd0e6]">{p.effect}</td>
