...
const CLASSES_AZ = [...DCC_CLASSES].sort((a, b) => a.name.localeCompare(b.name, "en"));
...
const CATEGORIES = [...new Set(DCC_CLASSES.flatMap((c) => c.categories))].sort(
  (a, b) => {
...
function matches(c: DccClass, q: string, cat: string): boolean {
...
export default async function DccClassesPage({
  ...
  const results = CLASSES_AZ.filter((c) => matches(c, needle, activeCat));
    ...
    <div className="mx-auto w-full max-w-6xl px-5 py-10">
      <header className="mb-8 flex items-end justify-between gap-4 border-b border-[var(--border)] pb-6">
        <div>
          ...
          <p className="mt-1 text-[13px] font-semibold uppercase tracking-[0.25em] text-[var(--red)] sm:text-[11px] sm:tracking-[0.35em]">
            {DCC_CLASSES.length} crawler classes
          </p>
        </div>
        <Link
          href="/dashboard"
      ...
      {results.length === 0 ? (
        ...
        <ul className="grid grid-cols-1 gap-3 md:grid-cols-2 items-start">
          {results.map((c, i) => (
            <li key={`${c.name}-${i}`} className="rounded-lg border border-[var(--border)] bg-[var(--panel)] p-4">
