...
type Query = { q?: string; group?: string };
...
const RACES_SORTED = [...DCC_RACES].sort(
  (a, b) => GROUP_RANK[a.group] - GROUP_RANK[b.group] || a.name.localeCompare(b.name, "en"),
);
...
function matches(r: DccRace, q: string, group: string): boolean {
...
export default async function DccRacesPage({
  ...
  const results = RACES_SORTED.filter((r) => matches(r, needle, activeGroup));
  const filtered = Boolean(needle || activeGroup);
  const current: Query = { q: q.trim(), group: activeGroup };
    ...
    <div className="mx-auto w-full max-w-6xl px-5 py-10">
      <header className="mb-8 flex items-end justify-between gap-4 border-b border-[var(--border)] pb-6">
        <div>
          ...
          <p className="mt-1 text-[13px] font-semibold uppercase tracking-[0.25em] text-[var(--red)] sm:text-[11px] sm:tracking-[0.35em]">
            {DCC_RACES.length} crawler races
          </p>
        </div>
        <Link
          href="/dashboard"
      ...
      {results.length === 0 ? (
        ...
        <ul className="grid grid-cols-1 gap-3 md:grid-cols-2 items-start">
          {results.map((r, i) => (
            <li key={`${r.name}-${i}`} className="rounded-lg border border-[var(--border)] bg-[var(--panel)] p-4">
