...
export default async function DndBestiaryPage({ searchParams }: { searchParams: Promise<RawQuery> }) {
  ...
  const groups = GROUP_ORDER.filter((g) => DND_MONSTERS.some((m) => m.group === g));
  const crs = [...new Set(DND_MONSTERS.map((m) => m.cr))].sort((a, b) => crVal(a) - crVal(b));

  const selected = pick ? DND_MONSTERS.find((m) => m.name === pick) ?? null : null;
  if (selected) {
    return (
  ...
  const current = { q, group, cr };
  const filtered = !!(q || group || cr);

  return (
    <div className="mx-auto w-full max-w-6xl px-5 py-10">
      <DndHeader title="Bestiary" subtitle={`${DND_MONSTERS.length} creatures · CR 0–30`} />
      <SearchForm base={BASE} q={q} placeholder="Search creatures by name or type…" hidden={{ group, cr }} />
        ...
        <ul className="grid grid-cols-1 items-start gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {list.map((m) => (
            <li key={m.name} className={cardCls}>
              <a href={withParams(BASE, current, { m: m.name })} className="text-base font-bold uppercase tracking-[0.1em] text-[#f0a37f] hover:underline">{m.name}</a>
