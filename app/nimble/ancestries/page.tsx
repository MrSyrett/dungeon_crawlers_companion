...
export default async function NimbleAncestriesPage({ searchParams }: { searchParams: Promise<RawQuery> }) {
  ...
  const anc = kind && kind !== "ancestry" ? [] : NIMBLE_ANCESTRIES.filter((a) => !needle || [a.name, a.description, ...a.traits.map((t) => t.name + " " + t.text)].join(" ").toLowerCase().includes(needle));
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-5 py-10">
      <NimbleHeader title="Ancestries & Backgrounds" subtitle={`${NIMBLE_ANCESTRIES.length} ancestries · ${NIMBLE_BACKGROUNDS.length} backgrounds · ${NIMBLE_MOTIVATIONS.length} motivations`} />
      ...
      {anc.length ? <ul className="grid grid-cols-1 items-start gap-3 md:grid-cols-2">{anc.map((a) => (
        <li key={a.name} className={cardCls}>
          <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1"><h2 className={nameCls}>{a.name}</h2><span className="text-[11px] uppercase tracking-[0.12em] text-[var(--muted)]">{a.size}</span><span className={badge}>{a.group} · p.{a.page}</span></div>
