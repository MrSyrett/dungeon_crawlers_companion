...
import type { AceGear } from "@/lib/data/ace-types";
import {
...
const TIERS = ["Free", "Normal", "Hard", "Herculean", "Impossible"].map((t) => ({ key: t, label: t }));
const CATEGORIES = [...new Set(ACE_GEAR.map((g) => g.category))].sort().map((c) => ({ key: c, label: c }));

function matches(g: AceGear, q: string, book: string, tier: string, cat: string): boolean {
...
export default async function AceGearPage({ searchParams }: { searchParams: Promise<RawQuery> }) {
  ...
  const results = ACE_GEAR.filter((g) => matches(g, needle, book, tier, cat));
  const filtered = Boolean(needle || book || tier || cat);

  return (
    <div className="mx-auto w-full max-w-6xl px-5 py-10">
      <AceHeader title="Gear" subtitle={`${ACE_GEAR.length} items · money isn't tracked`} />
      ...
      {results.length === 0 ? (
        ...
        <ul className="grid grid-cols-1 items-start gap-3 md:grid-cols-2">
          {results.map((g) => (
            <li key={`${g.setting}-${g.name}`} className={cardCls}>
              <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                ...
                <span className={bookBadge}>{settingName(g.setting)} · p.{g.page}</span>
              </div>
