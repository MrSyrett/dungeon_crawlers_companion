...
import { DarkSpaceHeader, DarkSpaceNav, SearchForm, ChipRow, CountLine, EmptyState, cardCls, nameCls, badge, one, type Query, type RawQuery } from "@/components/DarkSpaceRef";
...
export default async function DarkSpaceBackgroundsPage({ searchParams }: { searchParams: Promise<RawQuery> }) {
  ...
  const bg = kind && kind !== "background" ? [] : DS_BACKGROUNDS.filter((b) => !needle || (b.name + " " + b.text).toLowerCase().includes(needle));
  const mo = kind && kind !== "motivation" ? [] : DS_MOTIVATIONS.filter((m) => !needle || (m.name + " " + m.text + " " + m.startBonus + " " + m.effect).toLowerCase().includes(needle));
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-5 py-10">
      ...
      <DarkSpaceNav active="Backgrounds" />
      ...
      {bg.length ? (
        <ul className="grid grid-cols-1 items-start gap-2 md:grid-cols-2">
          {bg.map((b) => (
            <li key={b.n} className="rounded border border-[var(--border)] bg-[var(--panel)] px-3 py-2 text-[13px] leading-relaxed text-[var(--muted)]">
              <span className={badge}>{b.n}</span> <span className="font-semibold text-[var(--text)]">{b.name}.</span> {b.text}
            </li>
          ))}
        </ul>
      ) : null}
      ...
      {mo.length ? (
        <section className="mt-6">
          ...
          <ul className="grid grid-cols-1 items-start gap-3 md:grid-cols-3">
            {mo.map((m) => (
              <li key={m.code} className={cardCls}>
                <h3 className={nameCls}>{m.name}</h3>
                <p className="mt-2 text-[12px] italic leading-relaxed text-[var(--muted)]">{m.text}</p>
                <p className="mt-2 text-[12px] leading-relaxed text-[var(--text)]"><span className="font-semibold text-[#8fd6ea]">Start:</span> {m.startBonus}</p>
                <p className="mt-1 text-[12px] leading-relaxed text-[var(--text)]"><span className="font-semibold text-[#8fd6ea]">Luck:</span> {m.effect}</p>
              </li>
            ))}
          </ul>
        </section>
