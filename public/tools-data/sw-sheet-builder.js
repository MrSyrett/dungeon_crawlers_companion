...
(function () {
  ...
  function finish() {
    ...
    // A starting weapon becomes a LINKED pair: an Equipment row and an Attack row
    // sharing one link token (so the attack has no ✕ and deleting the gear removes
    // it), matching how weapons added from the store behave.
    let _bl = 0;
    keep.forEach(e => { const w = matchWeapon(e); if (w) { const link = 'lk' + (++_bl); const dmg = w.damageText || toCode(w.damage); wp.push({ name: w.name, skill: (skills().find(s => norm(s.name) === norm(w.skill || '')) || {}).name || '', damage: dmg, range: w.range || '', notes: e.toLowerCase() !== w.name.toLowerCase() ? e : (w.kind === 'Melee' ? 'melee' : ''), link: link }); gear.push({ name: w.name, note: [w.kind, dmg, w.range].filter(Boolean).join(' · '), link: link }); } else if (/credit/i.test(e)) return; else if (!ship.craft && /freighter|starfighter|fighter|speeder|skiff|shuttle|starship|swoop/i.test(e)) { const v = (typeof SW_VEHICLES !== 'undefined' ? SW_VEHICLES : []).find(x => new RegExp(x.name.replace(/[^a-z0-9 ]/gi, '').split(' ').slice(0, 2).join('.*'), 'i').test(e)); ship = { name: '', craft: v ? v.name : e, speed: v ? [v.speed, v.maneuverability].filter(Boolean).join(' · ') : '', hull: v ? [v.hull, v.shields].filter(Boolean).join(' · ') : '', weapons: v ? (v.weapons || []).map(w => [w.count, w.name, w.fireControl ? 'FC ' + w.fireControl : '', w.damage ? 'dmg ' + w.damage : ''].filter(Boolean).join(' ')).join('; ') : '' }; } else gear.push({ name: e, note: '' }); });
