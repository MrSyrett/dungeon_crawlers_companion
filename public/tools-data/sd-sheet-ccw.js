...
function ccwMethod() {
  ...
  h += '<label style="display:flex;align-items:center;gap:6px;font-family:Montserrat,sans-serif;font-size:11px;font-weight:700;color:'+(_ccw.heroDark?'#e0b83a':'#ccc')+';cursor:pointer;" title="HeroDark: max HP at level 1, choose talents on level-up, an even-level Skill Talent, unlimited Luck (and a Luck point on any natural 1 or 20), 3 actions a turn, and Dying instead of unconscious at 0 HP. Toggle it later on the sheet."><input type="checkbox" '+(_ccw.heroDark?'checked':'')+' onchange="_ccw.heroDark=this.checked;ccwRender()"> ⚔ HeroDark</label>';
  h += '</div>';
