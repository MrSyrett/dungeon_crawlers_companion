...
window.DungeonEngine = (function(){
  ...
  function drawObjects(){
    ...
    for(const o of map.objects){
      ...
      if(!tc && t.atlas) tc=_plainSprite(t);                    // untinted FA/bundled sprite → tiny cached canvas
      if(tc){ ctx.drawImage(tc, -w/2,-h/2, w,h); }
      else { const a=t.atlas;
        if(a) ctx.drawImage(t.img, a.x, a.y, a.w, a.h, -w/2, -h/2, w, h);  // fallback: atlas sub-rect
