...
window.DungeonEngine = (function(){
  ...
  function drawWallInnerShadow(){
    ...
    if(__needSeam || __hasNested){ const ov=oc();
      ...
      // nested room also has an EXTERIOR that sits on the container's floor — its
      // wall should throw a shadow OUTWARD too, like any structure standing in a
      // room. Build both rings for every nested room into one feathered mask,
      // unioned with "lighten" so overlaps never double, clip it to the floor,
      // then ERASE that footprint out of `c` before stamping it once — so a nested
      ...
      if(nested.length){
        ...
        for(const S of nested){
          const hpPx=halfPxFor(shapeWallTex(S)), reach=hpPx + px;
          // S solid — the shared base for both rings.
          rc.setTransform(1,0,0,1,0,0); rc.filter="none"; rc.globalCompositeOperation="source-over"; rc.clearRect(0,0,W,H);
          rc.fillStyle="#fff"; rc.beginPath(); shapePath(rc,S); rc.fill("nonzero");
          // INNER ring: S minus its inset, feathered inward.
          tc.setTransform(1,0,0,1,0,0); tc.filter="none"; tc.globalCompositeOperation="source-over"; tc.clearRect(0,0,W,H);
          tc.drawImage(run,0,0);
          tc.globalCompositeOperation="destination-out"; tc.filter="blur("+(px*0.38).toFixed(2)+"px)";
          tc.fillStyle="#fff"; fillInsetShape(tc, S, halfCellsFor(shapeWallTex(S)) + WALL_SHADOW_CELLS);
          tc.filter="none";
          nbc.globalCompositeOperation="lighten"; nbc.filter="none"; nbc.drawImage(tmp,0,0);
          // EXTERIOR ring: a thick stroke straddling the outline, minus S, so only
          // the outward half survives — the shadow cast onto the container floor.
          tc.setTransform(1,0,0,1,0,0); tc.filter="none"; tc.globalCompositeOperation="source-over"; tc.clearRect(0,0,W,H);
          tc.strokeStyle="#fff"; tc.lineJoin="round"; tc.lineWidth=2*reach; tc.beginPath(); shapePath(tc,S); tc.stroke();
          tc.globalCompositeOperation="destination-out"; tc.drawImage(run,0,0);
          nbc.globalCompositeOperation="lighten"; nbc.filter="blur("+(px*0.30).toFixed(2)+"px)"; nbc.drawImage(tmp,0,0); nbc.filter="none";
        }
        nbc.globalCompositeOperation="destination-in"; nbc.drawImage(buf.erode,0,0);          // keep only what lands on floor
        c.globalCompositeOperation="destination-out"; c.filter="none"; c.drawImage(nb,0,0);   // clear the footprint (kills doubling)
        nbc.globalCompositeOperation="source-in"; nbc.fillStyle="rgba(8,10,16,"+(0.78*amt).toFixed(3)+")"; nbc.fillRect(0,0,W,H);
        c.globalCompositeOperation="source-over"; c.drawImage(nb,0,0);
      } }
  ...
  function drawDecoShapes(){
    ...
    // shapes merge.
    { let anyTex=false;
      const db=oc(); db.width=W; db.height=H; const dbc=db.getContext("2d");
      dbc.setTransform(1,0,0,1,0,0); dbc.clearRect(0,0,W,H); dbc.fillStyle="#fff"; dbc.beginPath();
      for(const sh of map.shapes){ if(!sh.deco || !shHasFloor(sh)) continue;
        const ft=shapeFloorTex(sh); if(ft && ft.img && patFor(ctx,ft)){ shapePath(dbc, sh); anyTex=true; } }
      if(anyTex){ dbc.fill("nonzero");
        // Keep the opaque base ONLY where NO room floor lies beneath (over bare rock),
        // so a translucent deco floor (water, mud) laid over a room floor shows the room
        // floor through it — while a deco floor over rock still gets a base (no rock-through).
        dbc.globalCompositeOperation="destination-out"; dbc.drawImage(buf.mask,0,0);
        dbc.globalCompositeOperation="source-in"; dbc.fillStyle="#000"; dbc.fillRect(0,0,W,H);
    ...
    if(dW.length){
      const containsFully=shapeContains;
  ...
  function generate(opts){
    ...
    if(organic){
      ...
      while(house<nRooms && tries++<guard){
        // A MIX: about half the houses are little DWELLINGS (one footprint with
        // interior rooms nested inside), the rest shared-wall room clusters.
        const dwelling=rng()<0.5;
        const w=dwelling ? ri(Math.max(7,smin+2), smax+4) : ri(smin,smax);
        const h=dwelling ? ri(Math.max(7,smin+2), smax+4) : ri(smin,smax);
        ...
        const first={x,y,w,h,round:false,cx:x+w/2,cy:y+h/2,house};
        const mine=[first];
        if(dwelling){
          for(const K of collectBuildingRooms(first, ri(2,4), Math.max(3,smin-2), Math.min(smax, Math.min(w,h)-3)))
            nestedKids.push(K);
        } else {
        const extra=ri(1,4);                        // 2–5 rooms per house
      ...
      // vertical wall above and below it — cutting the floor into pie sections,
      // each with a door punched in its wall. Optionally one rect room, and a
      // stair by the top wall. No nested boxes floating in the middle.
      const R=Math.max(5, Math.round(Math.sqrt(Math.max(2,nRooms))*avg*0.55));
      rooms.push({x:-R,y:-R,w:2*R,h:2*R,round:true,cx:0,cy:0});
      const r2=v=>Math.round(v*100)/100, s2=v=>Math.round(v*2)/2;
      // horizontal chord wall
      const ry=ri(-Math.round(R*0.3), Math.round(R*0.3));
      const hx=r2(Math.sqrt(R*R-ry*ry)*0.99);
      map.walls.push({id:uid(), x1:-hx, y1:ry, x2:hx, y2:ry});
      map.doors.push({id:uid(), x:s2(ri(-Math.round(hx-2), Math.round(hx-3))+0.5), y:ry, a:0, len:2, _adj:true});
      // vertical wall from the top shell down to the chord
      const vx=ri(-Math.round(R*0.35), Math.round(R*0.35));
      const ty=r2(-Math.sqrt(R*R-vx*vx)*0.99);
      map.walls.push({id:uid(), x1:vx, y1:ty, x2:vx, y2:ry});
      map.doors.push({id:uid(), x:vx, y:s2((ty+ry)/2), a:Math.PI/2, len:1, _adj:true});
      // vertical wall from the chord down to the bottom shell
      const vx2=ri(-Math.round(R*0.35), Math.round(R*0.35));
      const by=r2(Math.sqrt(R*R-vx2*vx2)*0.99);
      map.walls.push({id:uid(), x1:vx2, y1:ry, x2:vx2, y2:by});
      map.doors.push({id:uid(), x:vx2, y:s2((ry+by)/2), a:Math.PI/2, len:(rng()<0.5?1:2), _adj:true});
      // one rect room straddling the partitions (60%), doors on two walls
      if(rng()<0.6){
        const kw=ri(4, Math.max(4,Math.min(6,R-1))), kh=ri(3, Math.max(3,Math.min(5,R-1)));
        const kx=ri(-Math.round(kw*0.7), 0), ky=ri(-Math.round(kh*0.7), 0);
        const m2=R-1.2;
        if(Math.hypot(kx,ky)<=m2 && Math.hypot(kx+kw,ky)<=m2 && Math.hypot(kx,ky+kh)<=m2 && Math.hypot(kx+kw,ky+kh)<=m2){
          const K={x:kx,y:ky,w:kw,h:kh,round:false,cx:kx+kw/2,cy:ky+kh/2};
          rooms.push(K);
          const bb={x:-R,y:-R,w:2*R,h:2*R};
          addNestedDoor(K, bb); addNestedDoor(K, bb);
        }
      }
      // stair by the top wall (like the example), skipped if a room covers it
      { const sy=r2(-R+1);
        const hit=rooms.some(k=>!k.round && 1>=k.x-0.5 && -1<=k.x+(k.w||0)+0.5 && sy+2>=k.y-0.5 && sy<=k.y+(k.h||0)+0.5);
        if(!hit) map.stairs.push({id:uid(), ax:1, ay:sy, bx:1, by:r2(sy+2), tx:-1, ty:r2(sy+1)}); }
    } else if(opts.ship){
      // SHIP (v3, modelled on the user's example): a RECT midsection with a
      // 45°-CHAMFERED stern and bow butted onto its ends (three rooms whose
      // shared seams merge into one hull), cabins subdividing the midsection
      // dwelling-style, a door on each seam, and a stair on the top deck.
      const B=Math.max(6, Math.round(Math.sqrt(Math.max(2,nRooms))*avg*0.55));
      const midW=Math.round(B*2.1);
      const c2=Math.max(1, Math.round(B*0.25));
      const sternL=Math.max(3, Math.round(B*0.75));
      const bowC=Math.max(2, Math.round(B*0.375)), bowL=Math.max(bowC+2, B-1);
      const shoulder=Math.max(1, bowL-bowC-1);
      const s2=v=>Math.round(v*2)/2;
      const mid={x:0, y:0, w:midW, h:B, round:false, cx:midW/2, cy:B/2};
      rooms.push(mid);
      rooms.push({pts:[[0,0],[0,B],[-(sternL-2),B],[-sternL,B-c2],[-sternL,c2],[-(sternL-2),0]],
                  x:-sternL, y:0, w:sternL, h:B, cx:-sternL/2, cy:B/2});
      rooms.push({pts:[[midW,0],[midW+shoulder,0],[midW+bowL,bowC],[midW+bowL,B-bowC],[midW+shoulder,B],[midW,B]],
                  x:midW, y:0, w:bowL, h:B, cx:midW+bowL/2, cy:B/2});
      // seam doors: mid↔stern and mid↔bow, like the example
      map.doors.push({id:uid(), x:0,    y:s2(B/2), a:Math.PI/2, len:2, _adj:true});
      map.doors.push({id:uid(), x:midW, y:s2(B/2), a:Math.PI/2, len:2, _adj:true});
      // cabins fill the midsection, anchored to its walls
      var nestedKids = collectBuildingRooms(mid, Math.max(2,nRooms-1), Math.max(2,smin-2), Math.min(smax, Math.max(4,B-2)));
      // stair on the top deck near the middle, skipped if a cabin covers it
      { const sx=s2(Math.round(midW*0.45))+0.5;
        const hit=nestedKids.some(k=> sx+1.5>=k.x-0.5 && sx-0.5<=k.x+k.w+0.5 && 2>=k.y-0.5 && 0<=k.y+k.h+0.5);
        if(!hit) map.stairs.push({id:uid(), ax:sx, ay:0, bx:sx, by:2, tx:sx+1.5, ty:1}); }
    } else if(structure){
